...
marketOpenLocalDelayed =  marketOpenLocal
marketCloseLocalDelayed =  marketCloseLocal
...
homePageFigures_ = [ homePageFig1, homePageFig2, marketsOpenClose]
...
def animateHomePage(i, pairIndex, figures):
	...
	if pairIndex == 2:
		...
		figures[0].text(0.05,0.25,exchange +'\n\nCurrent Time:'+datetime.now().astimezone().strftime('%Y-%m-%d %H:%M:%S%z')+ '\n\nOpen: '
			...
			+str(marketCloseLocalDelayed.astimezone()),bbox=props, verticalalignment='bottom')
...
app.geometry('%dx%d+%d+%d' % (ws, hs*fractionCovering, x, y))
